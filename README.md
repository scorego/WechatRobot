...
主要是想写一个群助手，作为在群里的工具使用。所以这个机器人监听的信息主要是以指令前缀开头的(默认是两个问号)。这样做的考虑是在群里人数比较多的时候，如果监听的是@自己的消息，那么在发送指令时@人太麻烦。另外，考虑到国内手机输入法的习惯，默认指令前缀是两个问号，因为拼音9宫格的布局问号在快捷栏里，方便输入。  
对于具体指令，希望汉字优先，缩写为主。
